...
export default function PendingFollowupsPage() {
  ...
  const loadFollowups = useCallback(async () => {
    try {
      ...
      console.log('Raw pending API response:', leadsWithFollowups);
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-200 via-orange-100 to-yellow-50 p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-3 mb-8">
          <svg className="w-8 h-8 text-blue-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
          <h1 className="text-4xl font-extrabold text-blue-900 tracking-tight">Pending Followups</h1>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 mb-8">
          ...
          <Button variant="secondary" className="rounded-full px-4 py-2 shadow border border-blue-200 hover:bg-blue-300">Export</Button>
          <span className="bg-blue-100 px-4 py-2 rounded-full text-blue-800 font-bold shadow border border-blue-200">Total: {filteredLeads.length}</span>
          <Input type="text" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)} className="px-4 py-2 rounded-full border border-blue-200 text-base focus:outline-none focus:ring-2 focus:ring-blue-200 bg-blue-50 text-blue-900 shadow" />
          <Button variant="secondary" className="rounded-full px-4 py-2 shadow border border-blue-200 hover:bg-blue-300" onClick={async () => {
            console.log('Manual refresh triggered');
            // Clear cached data and reload
            if (typeof window !== 'undefined') {
              localStorage.removeItem("followups");
          ...
          <Button variant="secondary" className="rounded-full px-4 py-2 shadow border border-blue-200 hover:bg-blue-300">Import</Button>
        </div>
        ...
        {loading && (
          <div className="text-center py-8">
            ...
            <p className="mt-2 text-blue-700">Loading pending followups...</p>
          </div>
        )}
        
        {/* Table Structure */}
        {!loading && (
          <Card className="overflow-x-auto rounded-2xl shadow-2xl border border-blue-100 bg-white/90">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
          <Card className="bg-blue-100 rounded-2xl p-8 text-center font-extrabold text-blue-800 shadow-md border border-blue-200 text-lg md:text-xl">Pending Count</Card>
          <Card className="bg-cyan-100 rounded-2xl p-8 text-center font-extrabold text-cyan-800 shadow-md border border-cyan-200 text-lg md:text-xl">Scheduled</Card>
          <Card className="bg-indigo-100 rounded-2xl p-8 text-center font-extrabold text-indigo-800 shadow-md border border-indigo-200 text-lg md:text-xl">In Progress</Card>
        </div>
      </div>
    </div>
  );
